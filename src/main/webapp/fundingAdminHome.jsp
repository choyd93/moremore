<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>펀딩관리자 | more more</title>
   	<link rel="stylesheet" href="resources/css/styles.css" type="text/css" />
</head>
<body>
	<%@include file="adminHeader.jsp" %>
	<div class="adminContainerWrapper">
		<div class="myPageContainerLeft">
			<ul class="adminLeftMenu">
				<li class="myPageMenuItem"><a href="#">메인</a></li>
				<li class="myPageMenuItem"><a href="#">펀딩 준비</a></li>
				<li class="myPageMenuItem"><a href="#">펀딩 관리</a></li>
				<li class="myPageMenuItem"><a href="#">결제 현황</a></li>
				<li class="myPageMenuItem"><a href="#">문의 관리</a></li>
			</ul>
		</div>
		<div class="adminContainerRightWrapper">
			<div class="adminContainerRight">
				<div class="adminLocationArea">
					<h8>메인</h8>
				</div>
				<div class="adminContainerContent">
				</div>
			</div>
		</div>
	</div>
</body>
</html>