							...
							<ul id="project_container_ul">
								...
								<c:forEach var="project" items="${project }">
									<li class="project_container_li pro_conta">
										<a href="updateProject.do?user_no=${project.user_no }&pro_no=${project.pro_no}&nowPage=1&min=1&max=10" id = "span_container_a">
											<span>${project.pro_title }</span>
											<span class="stitle">${project.pro_stitle }</span>
...
<script>
	...
	for (let i = startPage; i <= totalPage; i++) {
		...
		if (i === startPage){
			board.innerHTML += "<button class='btn_next_prev' onclick='page_back()'>이전으로</button>";
		}
		board.innerHTML += "<button class='btn_page' onclick='ajax_data(this)'>" + i + "</button>";
		if (i === totalPage){
			board.innerHTML += "<button class='btn_next_prev' onclick='page_go()'>다음으로</button>";
		}
	}
	page_go = () => {
	...
	function ajax_data (e) {
		if (e !== "NaN") {
			nowPage = e.innerHTML;
			console.log(document.querySelector(".btn_page"));
		} else {
			console.log(nowPage);
			nowPage = nowPage;	
		}
			...
			success: (data) => {
				...
				$.each(data,(idx, e) => {
					changedData += "<li class='project_container_li pro_conta'>"
						+ "<a href=updateProject.do?user_no=" + e.user_no + "&pro_no="+ e.pro_no + "&nowPage=" + nowPage + "&min=" + min + "&max=" + max 
						+ " class = span_container_a>"
						+ "<span>" + e.pro_title + "</span>"
						+ "<span class=stitle>" + e.pro_stitle + "</span>"
